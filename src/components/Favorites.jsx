...
const Favorites = () => {
    ...
    const eliminarProductoFavorito = (id) => {
        swal({
            title: "¿Quieres eliminar este producto de favoritos?",
                    ...
                    {favoritos.length ? (
                        favoritos.map(e =>
                            e.Productos.map((producto, index) => (
                                <div key={index} className={style.divContenedorCard}>
                                    <div className={style.card}>
                                        <div className={style.divBorrar}>
                                            <button
                                                title='Eliminar de favoritos'
                                                onClick={() => eliminarProductoFavorito(e.id)}
                                            >
                                                <svg className={style.svgTacho} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512">
                                                    <path d="M310.6 150.6c12.5-12.5 12.5-32.8 0-45.3s-32.8-12.5-45.3 0L160 210.7 54.6 105.4c-12.5-12.5-32.8-12.5-45.3 0s-12.5 32.8 0 45.3L114.7 256 9.4 361.4c-12.5 12.5-12.5 32.8 0 45.3s32.8 12.5 45.3 0L160 301.3 265.4 406.6c12.5 12.5 32.8 12.5 45.3 0s12.5-32.8 0-45.3L205.3 256 310.6 150.6z" />
                                                </svg>
                                            </button>
                                        </div>
                                        <Link className={style.link} to={`/product/${producto.id}`}>
                                        ...
                                        <button
                                            ...
                                            disabled={producto.cantidadDisponible === 0 && true}
                                        >
                                            + Añadir al carrito
                                        </button>
                                    </div>
                        ...
                        <p className={style.sinFavoritos}>No hay productos favoritos</p>
                    )}
                </div>
            </div>
        </div>
